(function($) {
	$(function() {
		...
		$item_list.find('li a').live('click', function(e) {
			...
			$details.load(SITE_URL + '/admin/content/pages/ajax_page_details/' + page_id, function(){
				refresh_sticky_page_details(true);
			});

			$details.siblings('h3').text(page_title);
		...
		post_sort_callback = function() {
			...
			if($details_id.val() > 0)
			{
				$details.load(SITE_URL + 'admin/content/pages/ajax_page_details/' + $details_id.val());
			}
		}

		// And off we go
		bonfire.sort_tree($item_list, $url, $cookie, data_callback, post_sort_callback);
